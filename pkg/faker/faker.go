package faker

import (
	"fmt"
	"math/rand"
)

var (
	numberRunes = []rune("0123456789")
	letterRunes = []rune("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
)

// Password equal password
const Password = "$2a$04$5vcLELOMsTLvXsUM9Nd.FegPIrNRT3s1mjEyH3rk2mh/b9iQGHLzG"


// 大小写混淆
func randStringRunes(n int) string {
	b := make([]rune, n)
	for i := range b {
		b[i] = letterRunes[rand.Intn(len(letterRunes))]
	}

	return string(b)
}

// 小写
func randStringLowerRunes(n int) string {
	b := make([]rune, n)
	for i := range b {
		b[i] = letterRunes[rand.Intn(len(letterRunes)/2)]
	}

	return string(b)
}


func RandInt(min, max int) int {
	return rand.Intn(max-min+1) + min
}

func RandIntSpec() string {
	num := RandInt(1, 999_999)
	// 0 表示填充字符是零，6 表示总宽度为 6 个字符，d 表示十进制整数
	return fmt.Sprintf("%06d", num)
}

func Username() string {
	return randStringRunes(RandInt(2, 10))
}

func ID() string {
	return fmt.Sprintf("%s-%s-%s-%s", randStringRunes(4), randStringRunes(4), randStringRunes(4), randStringRunes(4))
}

func UUID() string {
	return GenerateUUID()
}

func Email() string {
	return fmt.Sprintf("%s@example.com", randStringLowerRunes(RandInt(5, 10)))
}

func RandStr(n int) string {
	return randStringRunes(n)
}
